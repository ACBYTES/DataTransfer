...
namespace DataTransfer.src
{
    public static class Server
    {
        ...
        /// <summary>
        /// Calls <see cref="ServerFile.Cancel(string, bool)"/> on a possible <see cref="ServerFile"/> in <see cref="activeFiles"/>.
        /// </summary>
        ...
        public static void WriteToClient(byte[] Bytes, string FileName)
        {
            Task.Run(() =>
            {
                ...
                if (Bytes[0] != (byte)HeaderType.FileCancellationSignal)
                {
                    ...
                    Core.Threads.SleepUntil(() => { return activeFileNameProcessingDone; }); //If no processes confirm the end of the active file's processings, we release the mutex because the timeout processings are done by other threads and we don't need to request cancellation on the active file here.
                }
