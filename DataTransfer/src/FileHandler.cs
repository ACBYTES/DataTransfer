        ...
        public class ServerFile : IFile, IDisposable
        {
            ...
            private void SendFile()
            {
                try
                {
                    ...
                    {
                        Cancel(ConsoleManager.ConsoleResponses.RESPONSE_TIMEOUT, false); //Shouldn't send a cancellation signal because there were no confirmations. There is a possibility that the other side confirms after the timeout; in that case, they should wait until their local process times out.
                    }
                }
                catch (Exception Exc)
                {
