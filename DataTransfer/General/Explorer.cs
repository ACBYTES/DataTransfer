...
namespace DataTransfer.General
{
    ...
    public static class Explorer
    {
        ...
        public static void Show()
        {
            ...
            {
                values.Add(GenerateShape(ConsoleShapes.Rectangle, $"{i}: [{files[i].Replace(CurrentDir, "")}]", ""));
            }

            for (int i = 0; i < dirs.Length; i++)
            {
                values.Add(GenerateShape(ConsoleShapes.Rectangle, $"{i}: [{dirs[i].Replace(CurrentDir, "")}]", ""));
            }
