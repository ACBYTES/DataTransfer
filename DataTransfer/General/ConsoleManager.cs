...
namespace DataTransfer.General
{
    ...
    public static class ConsoleManager
    {
        ...
        public static ConsoleCommand[] ConsoleCommands { get; } = new ConsoleCommand[] { new ConsoleCommand("exit", new Action(() => Environment.Exit(0))),
            ...
            new ConsoleCommand("-reset", () => { (Core.CurrentMode == Core.UserMode.Client ? (Action)(() => { Client.Reset(); }) : Core.CurrentMode == Core.UserMode.Server ? Server.Reset : () => { WriteShape(ConsoleColor.Gray, ConsoleShapes.Rectangle, "There's nothing to reset."); })(); })
        };
